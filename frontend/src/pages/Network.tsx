...
export default function Network() {
  ...
  return (
    <div className="page network-page">
      ...
      <section className="network-device-strip"><span><BatteryCharging size={14} /> iQOO {battery ?? '--'}%</span><span><Bluetooth size={14} /> BLE {bluetoothState}</span><span className={connected ? 'is-live' : 'is-muted'}><span className="pulse-dot" /> {connected ? 'SSE LINKED' : 'SSE WAITING'}</span></section>
      ...
      <section className="network-status-card">
        ...
        <div className="network-status-sub"><span>{readiness.reason}</span><strong>{online ? 'BACKEND LINK READY' : 'OFFLINE QUEUE ACTIVE'}</strong></div>
