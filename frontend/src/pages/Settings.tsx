...
import { ArrowLeft, Bluetooth, CheckCircle2, ChevronRight, Copy, RadioTower, RotateCcw, ShieldCheck, Smartphone, UserRound, Sun, Moon, MonitorCog } from 'lucide-react';
...
import { disableWebPush, enableWebPush } from '../state/push';
...
export default function Settings() {
  ...
  const { user, device, logout, sendChangeCode, changePassword } = useSession();
  ...
  return <div className="hardware-settings">
    ...
    <Section icon={<UserRound size={17} />} title="Operator & Device" badge={user ? 'SIGNED IN' : 'LOCAL MODE'}><div className="operator-grid"><div><small>OPERATOR</small><strong>{user?.displayName ?? 'LOCAL OPERATOR'}</strong><span>{user?.email ?? user?.role?.toUpperCase() ?? 'EMERGENCY PROFILE STORED LOCALLY'}</span>{user?.phone && <span>{user.phone}</span>}</div><div className="hardware-fingerprint"><small>DEVICE ID</small><code>{device?.publicId ?? 'LOCAL DEVICE'}</code><button type="button" disabled={!device?.publicId} onClick={() => void navigator.clipboard?.writeText(device?.publicId ?? '')}><Copy size={13} /> COPY</button></div></div>{user && <button className="hardware-signout" type="button" onClick={logout}>Sign out</button>}</Section>

    {user && <Section icon={<ShieldCheck size={17} />} title="Account security" badge="EMAIL + SMS"><p className="hardware-scale">Password changes require your current password and a 6-digit code sent to {user.email}{user.phone ? ` and ${user.phone}` : ''}.</p><label htmlFor="cur-pass">Current password</label><input id="cur-pass" type="password" value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} autoComplete="current-password" /><div className="grid2"><div><label htmlFor="chg-code">Code</label><input id="chg-code" value={changeCode} onChange={(event) => setChangeCode(event.target.value.replace(/\D/g, '').slice(0, 6))} inputMode="numeric" maxLength={6} /></div><div><label htmlFor="new-pass">New password</label><input id="new-pass" type="password" value={nextPassword} onChange={(event) => setNextPassword(event.target.value)} autoComplete="new-password" /></div></div><div className="hardware-transport"><ActionButton variant="secondary" disabled={passwordBusy} onClick={() => { setPasswordBusy(true); setPasswordStatus(''); sendChangeCode().then(() => setPasswordStatus('Code sent to email and phone.')).catch((error: unknown) => setPasswordStatus(error instanceof Error ? error.message : 'Could not send code')).finally(() => setPasswordBusy(false)); }}>Send code</ActionButton><ActionButton variant="primary" disabled={passwordBusy} onClick={() => { setPasswordBusy(true); setPasswordStatus(''); changePassword(currentPassword, changeCode, nextPassword).then(() => { setPasswordStatus('Password updated.'); setCurrentPassword(''); setChangeCode(''); setNextPassword(''); }).catch((error: unknown) => setPasswordStatus(error instanceof Error ? error.message : 'Could not change password')).finally(() => setPasswordBusy(false)); }}>Change password</ActionButton></div>{passwordStatus && <p className="ok-text">{passwordStatus}</p>}</Section>}

    {user && <Section icon={<Smartphone size={17} />} title="Browser notifications" badge="OPTIONAL"><p className="hardware-scale">Enable Web Push on this device for emergency alerts when the ResQNET tab is not open. Delivery is available only when the server has VAPID keys configured.</p><div className="hardware-transport"><ActionButton variant="secondary" disabled={pushBusy} onClick={() => { setPushBusy(true); setPushStatus(''); enableWebPush().then(setPushStatus).catch((error: unknown) => setPushStatus(error instanceof Error ? error.message : 'Could not enable browser notifications')).finally(() => setPushBusy(false)); }}>Enable browser push</ActionButton><ActionButton variant="ghost" disabled={pushBusy} onClick={() => { setPushBusy(true); setPushStatus(''); disableWebPush().then(setPushStatus).catch((error: unknown) => setPushStatus(error instanceof Error ? error.message : 'Could not disable browser notifications')).finally(() => setPushBusy(false)); }}>Disable</ActionButton></div>{pushStatus && <p className="hardware-scale" role="status">{pushStatus}</p>}</Section>}
    ...
    <section className="hardware-footer-controls"><div><small>APPEARANCE</small><div className="row wrap" role="radiogroup" aria-label="Color theme">{([['system', MonitorCog], ['light', Sun], ['dark', Moon]] as Array<[ThemePreference, typeof Sun]>).map(([theme, Icon]) => <ActionButton key={theme} variant={settings.theme === theme ? 'primary' : 'ghost'} onClick={() => settings.update({ theme })} ariaLabel={`${theme} theme`}><Icon size={16} /> {theme}</ActionButton>)}</div></div><ActionButton variant="secondary" onClick={settings.reset}><RotateCcw size={14} /> Reset relay settings</ActionButton></section>
  </div>;
}
